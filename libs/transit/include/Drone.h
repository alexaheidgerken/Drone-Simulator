...
class Drone : public IEntity {
 public:
  ...
  /**
   * @brief Set availability of the drone
   * @param choice boolean value of availability
   */
  void SetAvailability(bool choice) { available = choice; }

  /**
   * @brief Set the picked up status of the drone
   *
   * @param pickedUp The new picked up status of the drone
   */
  void SetPickedUp(bool pickedUp) override { this->pickedUp = pickedUp; }
  ...
  /**
   * @brief Sets the toRobot strategy of the drone
   * @param strategy The new toRobot strategy of the drone
   */
  void SetToRobot(std::string strategyName, Vector3 start,
                  Vector3 end) override;

  /**
   * @brief Sets the toFinalDestination strategy of the drone
   * @param strategyName The name of the strategy for travelling to the final
   * destination
   * @param start The starting position of the drone
   * @param end The ending position of the drone
   */
  void SetToDestination(std::string strategyName, Vector3 start,
                        Vector3 end) override;
   ...
   * @param entity
   */
  void SetNearestEntity(IEntity* entity) override { nearestEntity = entity; }
